...
from .models import Profile, Rating, Match, Message
from django.contrib.auth import login as auth_login
from django.contrib.auth import logout as auth_logout

def home(request):
    return render(request, 'taffy/home.html')
...
def signup(request):
    if request.method == 'POST':
        ...
        form = SignUpForm()
    return render(request, 'taffy/signup.html', {'form': form})    
...
def rate(request, ratedUserID = 0):
    ...
    try:
        ...
        try:
            ...
            return render(request, 'taffy/rate.html', {'user': userToRate.user})
        except AttributeError:
            return render(request, 'taffy/noProfilesToRate.html')
...
def profile(request, username):
    return render(request, 'taffy/profile.html', {'user': User.objects.all().get(username=username)})

@login_required
def selfProfile(request):
    return render(request, 'taffy/profile.html', {'user': request.user})

@login_required
def matches(request):
    matchesList = getMatches(request.user.profile)
    return render(request, 'taffy/matches.html', {'matches': matchesList})

@login_required
def mailbox(request):
    matchesList = getMatches(request.user.profile)
    return render(request, 'taffy/mailbox.html', {'matches': matchesList})
...
def menu(request):
    return render(request, 'taffy/menu.html')
